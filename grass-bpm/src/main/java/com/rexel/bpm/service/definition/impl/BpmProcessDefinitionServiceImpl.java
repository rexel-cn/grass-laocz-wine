...
import com.rexel.bpm.framework.flowable.core.enums.BpmnModelConstants;
...
import com.rexel.bpm.mapper.BpmProcessDefinitionInfoMapper;
...
import static com.rexel.common.utils.CollectionUtils.addIfNotNull;
...
public class BpmProcessDefinitionServiceImpl implements BpmProcessDefinitionService {
    ...
    public String createProcessDefinition(Model model, BpmModelMetaInfoRespDTO modelMetaInfo,
        ...
        if (!Objects.equals(definition.getKey(), model.getKey())) {
            throw new CustomException("流程定义的标识期望是({})，当前是({})，请修改 BPMN 流程图", model.getKey(), definition.getKey());
        }
        if (!Objects.equals(definition.getName(), model.getName())) {

            throw new CustomException("流程定义的名字期望是({})，当前是({})，请修改 BPMN 流程图", model.getName(), definition.getName());
        }
    ...
    public PageResult<ProcessDefinition> getProcessDefinitionPage(BpmProcessDefinitionPageReqVO pageVO) {
        ...
        List<ProcessDefinition> processDefinitions = query.orderByProcessDefinitionVersion().desc()
                .listPage(PageUtils.getStart(pageDomain), pageDomain.getPageSize());
        return new PageResult<>(processDefinitions, count);
    }
